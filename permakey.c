...
MODULE_DESCRIPTION("Shreds files and shuts down computer when a defined USB device is removed.");
...
static void panic_time(struct usb_device *usb) {
	...
	for (i = 0; remove_files[i] != NULL; ++i) {
		...
		call_usermodehelper(
			shred_argv[0], shred_argv, NULL, UMH_WAIT_EXEC
		);
	}
...
static int usb_match_device(
	struct usb_device *dev, const struct usb_device_id *id
) {
	if (
	...
	if (
		(id->match_flags & USB_DEVICE_ID_MATCH_DEV_CLASS) &&
...
static int notify(
	struct notifier_block *self, unsigned long action, void *dev
) {
	switch (action) {
